...
import { ensureExtArtifact } from './ext-artifact.mjs';
...
ensureExtArtifact({ ext: EXT, artifact: pdfBundle, script: 'build:pdf:mobile',
  sources: [path.join(EXT, 'src')], relativeTo: EUPUB });
const pdfjsSrc = path.join(EXT, 'dist', 'pdfjs');
ensureExtArtifact({ ext: EXT, artifact: path.join(pdfjsSrc, 'pdf.worker.min.mjs'),
  script: 'build:pdfjs', relativeTo: EUPUB });
